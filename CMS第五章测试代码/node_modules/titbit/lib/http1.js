...
class http1 {
  ...
  context () {
    ...
    ctx.moveFile = async (upf, options) => {
      return helper.moveFile(ctx, upf, options);
    };

    ctx.send = (data, ctype='text/plain; charset=utf-8', encoding='utf8') => {
    ...
    ctx.html = (data) => {
      ctx.send(data, 'text/html; charset=utf-8');
    };

    ctx.sendb = (data, ctype='application/ocect-stream') => {
      ctx.response.write(
        (data instanceof Buffer || typeof data === 'string') 
        ? data : JSON.stringify(data),
        'binary',
  ...
  globalLog (method, rinfo) {
    ...
    if (process.send && typeof process.send === 'function') {
      ...
      if (log_data.success) {
        ...
        console.error(JSON.stringify(log_data));
      }
    }
  }
